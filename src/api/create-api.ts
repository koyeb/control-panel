...
export function createApi(options: CreateApiOptions) {
  const { endpoint, stream } = createEndpoint(options);
...
function createEndpoint({ baseUrl, getToken }: CreateApiOptions) {
  ...
  function stream<Path extends StreamPath>(path: Path) {
    return function (params: StreamParams<Path>): WebSocket {
