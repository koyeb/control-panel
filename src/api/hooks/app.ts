  ...
  return useQuery({
    ...
    select(results): AppFull[] {
      ...
      const deployments = new Map(
        results.deployments.map(mapDeployment).map((deployment) => [deployment.serviceId, deployment]),
      );
      ...
      return apps.map((app) => ({
        ...
        services: services
          ...
          .map((service) => ({ ...service, latestDeployment: defined(deployments.get(service.id)) })),
      }));
    },
  });
}
