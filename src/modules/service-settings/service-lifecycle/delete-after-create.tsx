...
const T = createTranslate('pages.service.settings.lifecycle.deleteAfterCreate');
...
export function DeleteAfterCreate({ form }: { form: UseFormReturn<ServiceLifecycleFormType> }) {
  ...
  return (
    <div className="row items-start gap-4 rounded-md border px-3 py-2">
      ...
      <ControlledSwitch
        control={form.control}
        name="deleteAfterCreate.enabled"
        onChangeEffect={(event) => !event.target.checked && form.setValue('deleteAfterCreate.value', NaN)}
      />
    </div>
  );
}
